...
/* 在Nachos的原始代码中，下面一行代码的(position > fileLength) 这一部分代码本来是：
(position >= fileLength)。这对一上来就把文件长度固定，且无法扩展文件长度的实现，是完全符合
WriteAt方法的逻辑功能的。
   但对我们要实现的，随着写入动态改变文件长度的情况，就不能加等号=，来作为非法的入口参数判断了。
因为入口时，position等于fileLength是正常的 */
    if ((numBytes <= 0) || (position > fileLength))
	    return 0;				// check request
